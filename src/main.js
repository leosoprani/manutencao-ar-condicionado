...
async function renderDashboard(sortBy = 'proximas') {
  ...
  headerContent.innerHTML = `
    <div style="display: flex; flex-direction: column; gap: 15px;">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <h2 style="font-size: 20px;">Agenda de Serviços</h2>
        <select id="dash-sort" class="form-control" style="width: auto; font-size: 11px; padding: 4px 8px; height: 32px; background: var(--surface-container); border: none;">
          <option value="proximas" ${sortBy === 'proximas' ? 'selected' : ''}>Próximas</option>
          <option value="alerta" ${sortBy === 'alerta' ? 'selected' : ''}>Atrasados</option>
      ...
      <div class="search-bar-global" style="position: relative;">
        ...
        <input type="text" id="global-search" placeholder="Pesquisar cliente ou prédio..." style="width: 100%; background: rgba(255,255,255,0.05); border-radius: 12px; padding: 12px 15px 12px 40px; color: white; font-size: 14px; outline: none; border: 1px solid rgba(0, 242, 255, 0.1);">
      </div>
    </div>
  `;
  ...
  if (sortBy === 'bairro') {
    ...
    const displayEqs = sortBy === 'alerta' ? sortedEqs.filter(e => (new Date(e.proximaManutencao) - new Date()) < 0) : sortedEqs;

    if (displayEqs.length === 0) {
      html += '<div style="grid-column: span 2; text-align: center; padding: 60px 20px; opacity: 0.5;"><span class="material-symbols-rounded" style="font-size: 48px; display: block; margin-bottom: 10px;">calendar_today</span><p>Nenhuma manutenção pendente</p></div>';
    }
      ...
      html += `
        <div class="card" style="grid-column: span 2; display: flex; flex-direction: column; gap: 12px; background: ${diffDays <= 5 ? 'rgba(0, 242, 255, 0.04)' : 'var(--surface-container)'}; border: 1px solid ${diffDays <= 5 ? 'rgba(0, 242, 255, 0.2)' : 'var(--glass-border)'};">
          <div style="display: flex; align-items: center; gap: 15px;">
            ...
            <div style="flex: 1;" onclick="window.renderBairroDetail(${c?.bairroId}, 'home')">
              ...
              <div style="display: flex; align-items: center; gap: 5px; margin-top: 4px;">
                <span style="font-size: 10px; font-weight: 800; color: ${diffDays < 0 ? 'var(--accent)' : 'var(--primary)'}; background: rgba(0,242,255,0.1); padding: 2px 6px; border-radius: 4px;">${statusText}</span>
              </div>
            </div>
          </div>
          ...
          <div style="display: flex; gap: 8px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 12px;">
            ...
            <button class="icon-btn" style="background: rgba(255,255,255,0.05); width: 38px; height: 38px;" onclick="window.renderEquipmentForm(${eq.id}, ${c?.id})"><span class="material-symbols-rounded" style="font-size: 18px;">edit</span></button>
          </div>
        </div>
      `;
    }
...
async function renderEquipmentForm(id = null, preCId = null) {
  ...
  const renderForm = () => {
    modalBody.innerHTML = `
      <form id="f-e">
        ...
        <div class="brand-grid" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 25px;">
          ${marcas.map(m => `
            <div class="brand-item ${selectedBrand === m ? 'active' : ''}" data-brand="${m}" style="padding: 10px; border: 2px solid ${selectedBrand === m ? 'var(--primary)' : 'transparent'}; border-radius: 12px; text-align: center; cursor: pointer; background: ${selectedBrand === m ? 'rgba(0,242,255,0.1)' : 'rgba(255,255,255,0.02)'}; transition: 0.2s;">
        ...
        <div class="form-group"><label>Modelo / Referência</label><input type="text" id="e-m" class="form-control" value="${eq?.modelo || ''}" placeholder="Ex: Inverter WindFree"></div>
        <div class="form-group"><label>Local de Instalação</label><input type="text" id="e-l" class="form-control" value="${eq?.localizacao || ''}" placeholder="Ex: Sala, Recepção, Portaria" list="loc-list"><datalist id="loc-list"><option value="Sala"><option value="Quarto"><option value="Suíte"><option value="Recepção"><option value="Portaria"><option value="Salão de Festas"><option value="Academia"></datalist></div>
        <button type="submit" class="btn-primary" style="margin-top: 10px; padding: 15px; font-weight: 800;">SALVAR ALTERAÇÕES</button>
      </form>
    `;
    ...
    document.getElementById('f-e').onsubmit = async (e) => {
      e.preventDefault();
      const cId = preCId || eq?.clienteId || (cls.length > 0 ? cls[0].id : null);
      if (!cId) { alert('Selecione um cliente primeiro!'); return; }
      const data = { 
        marca: selectedBrand, 
      ...
      if (id) await db.equipamentos.update(id, data); 
      else await db.equipamentos.add({ ...data, ultimaManutencao: new Date(), proximaManutencao: new Date(Date.now() + 30*24*60*60*1000) });
  ...
  const renderMaint = () => {
    modalBody.innerHTML = `
      <div style="display: flex; gap: 10px; margin-bottom: 20px; background: rgba(255,255,255,0.03); padding: 5px; border-radius: 12px;">
        <button class="tab-btn ${tab === 'existente' ? 'active' : ''}" id="tab-ex" style="flex: 1; padding: 10px; border-radius: 8px; border: none; font-size: 12px; font-weight: 700; background: ${tab === 'existente' ? 'var(--primary)' : 'transparent'}; color: ${tab === 'existente' ? 'black' : 'white'};">EXISTENTE</button>
        <button class="tab-btn ${tab === 'avulso' ? 'active' : ''}" id="tab-av" style="flex: 1; padding: 10px; border-radius: 8px; border: none; font-size: 12px; font-weight: 700; background: ${tab === 'avulso' ? 'var(--primary)' : 'transparent'}; color: ${tab === 'avulso' ? 'black' : 'white'};">AVULSO (NOVO)</button>
      </div>
      
      <form id="f-m">
        ${tab === 'existente' ? `
          <div class="form-group">
            <label>Escolha o Aparelho</label>
            <select id="m-eq-id" class="form-control">
              ${eqs.map(e => { 
                const c = cls.find(x => x.id === e.clienteId); 
                return `<option value="${e.id}" ${eqId == e.id ? 'selected' : ''}>${c?.nome} - ${e.marca} (${e.localizacao})</option>`; 
              }).join('')}
            </select>
          </div>
        ` : `
          <div class="form-group"><label>Nome do Cliente / Edifício</label><input type="text" id="av-nome" class="form-control" required placeholder="Ex: João da Silva"></div>
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
            <div class="form-group"><label>Bairro</label><select id="av-b" class="form-control">${bairros.map(b => `<option value="${b.id}">${b.nome}</option>`).join('')}</select></div>
            <div class="form-group"><label>Marca</label><select id="av-m" class="form-control">${marcas.map(m => `<option value="${m}">${m}</option>`).join('')}</select></div>
          </div>
        `}
        
        <div class="form-group"><label>O que foi feito?</label><textarea id="m-desc" class="form-control" rows="3" required placeholder="Relate o serviço executado..."></textarea></div>
        <div class="form-group"><label>Data da Próxima Manutenção</label><input type="date" id="m-next" class="form-control" required value="${new Date(Date.now() + 180*24*60*60*1000).toISOString().split('T')[0]}"></div>
        <button type="submit" class="btn-primary" style="margin-top: 10px; padding: 15px; font-weight: 800;">FINALIZAR E SALVAR</button>
      </form>
    `;
    ...
    document.getElementById('f-m').onsubmit = async (e) => {
      ...
      if (tab === 'avulso') {
        const cId = await db.clientes.add({ nome: document.getElementById('av-nome').value, bairroId: Number(document.getElementById('av-b').value), endereco: 'Cadastro Avulso', whatsapp: '(83) 9', tipo: 'Residência', telefone: '(83) 9' });
        ...
        finalEqId = Number(document.getElementById('m-eq-id').value);
      }
...
async function renderHistorico() {
  ...
  if (os.length === 0) {
    html += '<div style="text-align: center; padding: 60px; opacity: 0.5;"><span class="material-symbols-rounded" style="font-size: 48px;">history</span><p>Nenhum serviço registrado</p></div>';
  }
  ...
  for (const m of os) {
    ...
    html += `
      <div class="card" style="display: flex; flex-direction: column; gap: 10px; border-left: 4px solid var(--secondary);">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
          <div>
            <span style="font-size: 10px; color: var(--primary); font-weight: 800; text-transform: uppercase;">Finalizado em ${new Date(m.dataRealizada).toLocaleDateString('pt-BR')}</span>
            <h4 style="margin: 4px 0; font-size: 15px;">${cli?.nome}</h4>
          </div>
          <span style="font-size: 10px; opacity: 0.5;">ID #${m.id}</span>
        </div>
        <div style="display: flex; align-items: center; gap: 8px; font-size: 11px; opacity: 0.8;">
          <img src="${getLogo(eq?.marca)}" style="width: 16px; height: 16px; object-fit: contain; filter: grayscale(1);" />
          <span>${eq?.marca} - ${eq?.localizacao}</span>
        </div>
        <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 10px;">
          <p style="font-size: 13px; font-style: italic; margin: 0; line-height: 1.4;">"${m.descricao}"</p>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 5px;">
           <span style="font-size: 10px; opacity: 0.6;">Próximo retorno: ${new Date(m.proximaData).toLocaleDateString('pt-BR')}</span>
           <button class="icon-btn" style="width: 32px; height: 32px;" onclick="alert('Relatório PDF em desenvolvimento!')"><span class="material-symbols-rounded" style="font-size: 18px;">description</span></button>
        </div>
      </div>
    `;
  }
...
function renderMais() {
  ...
  headerContent.innerHTML = `<h2>Mais</h2><p>Configurações</p>`;
...
// Missing Modals (Bairro and Property)
async function renderBairroForm() {
  openModal('Novo Bairro');
  modalBody.innerHTML = `<form id="f-b">
    <div class="form-group"><label>Nome do Bairro</label><input type="text" id="b-nome" class="form-control" required></div>
    <div class="form-group"><label>Cor</label><input type="color" id="b-cor" class="form-control" value="#00f2ff"></div>
    <button type="submit" class="btn-primary">Salvar</button>
  </form>`;
  document.getElementById('f-b').onsubmit = async (e) => {
    e.preventDefault();
    await db.bairros.add({ nome: document.getElementById('b-nome').value, cor: document.getElementById('b-cor').value });
...
async function renderPropertyForm() {
  ...
  modalBody.innerHTML = `<form id="f-p">
    <div class="form-group"><label>Nome da Propriedade/Cliente</label><input type="text" id="p-nome" class="form-control" required></div>
    ...
    <div class="form-group"><label>Endereço</label><input type="text" id="p-end" class="form-control" required></div>
    <div class="form-group"><label>WhatsApp</label><input type="text" id="p-wa" class="form-control" value="(83) 9"></div>
    <button type="submit" class="btn-primary">Salvar</button>
  </form>`;
  document.getElementById('f-p').onsubmit = async (e) => {
    e.preventDefault();
    await db.clientes.add({ nome: document.getElementById('p-nome').value, bairroId: Number(document.getElementById('p-b').value), endereco: document.getElementById('p-end').value, whatsapp: document.getElementById('p-wa').value, tipo: 'Residência', telefone: '(83) 9' });
    closeModal(); renderBairros();
  };
}
...
async function init() {
  ...
  if (localStorage.getItem('jampa_reset_v9') !== 'done') {
    await db.delete(); await db.open();
    await seedDatabase();
    localStorage.setItem('jampa_reset_v9', 'done');
  }
